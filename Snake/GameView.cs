...
public class GameView
{
    ...
    public void DrawMap(RenderWindow window)
    {
        ...
        for (int y = 0; y < _gameBoard.Size.Y; y++)
        {
            ...
            // Правая граница
            _wallSprite.Position = new Vector2f((_gameBoard.Size.X - 1) * _gameViewSettings.CellSize, y * _gameViewSettings.CellSize);
            window.Draw(_wallSprite);
        }
        ...
        for (int x = 0; x < _gameBoard.Size.X; x++)
        {
            ...
            // Правая граница 
            _wallSprite.Position = new Vector2f(x * _gameViewSettings.CellSize,    (_gameBoard.Size.Y - 1) * _gameViewSettings.CellSize);
            window.Draw(_wallSprite);
        }
    }
    ...
    private void DrawSnake(RenderWindow window)
    {
        for (int i = 0; i < _snake.GetSize(); i++)
        {
            ...
            if (i == 0)
            {
                ...
                _snakeHeadSprite.Position = new Vector2f(_snake.GetPoint(i).X * _gameViewSettings.CellSize, _snake.GetPoint(i).Y * _gameViewSettings.CellSize); 
            ...
            {
                // Наши координаты умножаем на размер ячейки
                _snakeBodySprite.Position = new Vector2f(_snake.GetPoint(i).X * _gameViewSettings.CellSize, _snake.GetPoint(i).Y * _gameViewSettings.CellSize); 
